...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    conv3_weights = tf.Variable(
        tf.truncated_normal([3, 3, 64, 256],  # Conv 3*3*256.
                            stddev=0.1,
                            seed=SEED))
    ...
    conv3_weights = tf.get_variable('conv3_weights', 
        shape=(3, 3, 64, 256), initializer=tf.contrib.layers.xavier_initializer()) 
    ...
    def model(data, train=False):
        ...
        conv2 = tf.nn.conv2d(pool1,
                            ...
                            strides=[1, 1, 1, 1],
                            padding='SAME')
        relu2 = max_out(tf.nn.bias_add(conv2, conv2_biases), 
            int(conv2.get_shape().as_list()[-1]/2), axis=None)
    ...
    with tf.Session(config = tf.ConfigProto(inter_op_parallelism_threads = NUM_THREADS, intra_op_parallelism_threads = NUM_THREADS)) as s:
        ...
        for i in range(1,TRAINING_SIZE+1):
            ...
            img_3c = convertToPNG(p_img)
            Image.fromarray(img_3c).save(dir +'satImage_'+str(i)+'.png')
