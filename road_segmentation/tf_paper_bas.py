...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    train_data_node = tf.placeholder(
        tf.float32,
        shape=(BATCH_SIZE, IMG_PATCH_SIZE, IMG_PATCH_SIZE, NUM_CHANNELS)) # X_batch
    ...
    conv1_weights = tf.Variable(
        tf.truncated_normal([9, 9, NUM_CHANNELS, 32],  
                            stddev=0.1,
                            seed=SEED))
    conv1_biases = tf.Variable(tf.zeros([32]))

    conv2_weights = tf.Variable(
        tf.truncated_normal([7, 7, 16, 64],
                            stddev=0.1,
                            seed=SEED))
    ...
    fc1_weights = tf.Variable(  # fully connected, depth 512.
        tf.truncated_normal([int(IMG_PATCH_SIZE / 2 * IMG_PATCH_SIZE / 2 * 32), 512],
                            stddev=0.1,
                            seed=SEED))
    fc1_biases = tf.Variable(tf.constant(0.1, shape=[512]))

    # add probability for drop out
    ...
    # Above to design the CNN structure
    ...
    def model(data, train=False):
        ...
        maxout1 = max_out(tf.nn.bias_add(conv1, conv1_biases), int(conv1.get_shape().as_list()[-1]/2), axis=None)

        pool1 = tf.nn.max_pool(maxout1,ksize=[1, 2, 2, 1],
                              strides=[1, 2, 2, 1],
                              padding='SAME')
        ...
        maxout2 = max_out(tf.nn.bias_add(conv2, conv2_biases), int(conv2.get_shape().as_list()[-1]/2), axis=None)


        conv3 = tf.nn.conv2d(maxout2,
                            conv3_weights,
                            strides=[1, 1, 1, 1],
                            padding='SAME')

        maxout3 = max_out(tf.nn.bias_add(conv3, conv3_biases), int(conv3.get_shape().as_list()[-1]/2), axis=None)

        max_shape = maxout3.get_shape().as_list()
        reshape = tf.reshape(
            maxout3,
            [max_shape[0], max_shape[1] * max_shape[2] * max_shape[3]])
        ...
        print ('maxout1 ' + str(maxout1.get_shape()))
        print ('pool1 ' + str(pool1.get_shape()))
        print ('conv2 ' + str(conv2.get_shape()))
        print ('maxout2 ' + str(maxout2.get_shape()))
        print ('conv3 ' + str(conv3.get_shape()))
        print ('maxout3 ' + str(maxout3.get_shape()))
        ...
        if train == True:
            ...
            s_conv1 = get_image_summary(conv1)
            filter_summary1 = tf.summary.image('summary_conv' + summary_id, s_conv1)
            
            s_max1 = get_image_summary(maxout1)
            filter_summary2 = tf.summary.image('summary_conv' + summary_id, s_max1)

            s_pool1 = get_image_summary(pool1)
            filter_summary3 = tf.summary.image('summary_pool' + summary_id, s_pool1)

            s_conv2 = get_image_summary(conv2)
            filter_summary4 = tf.summary.image('summary_conv' + summary_id, s_conv2)

            s_max2 = get_image_summary(maxout2)
            filter_summary5 = tf.summary.image('summary_conv' + summary_id, s_max2)

            s_conv3 = get_image_summary(conv3)
            filter_summary6 = tf.summary.image('summary_conv' + summary_id, s_conv3)

            s_max3 = get_image_summary(maxout3)
            filter_summary7 = tf.summary.image('summary_conv' + summary_id, s_max3)
        ...
        return out
        '''
    # Above to design the forward computation
    ...
    all_params_node = [conv1_weights, conv1_biases, conv2_weights, conv2_biases,conv3_weights, 
                            conv3_biases, fc1_weights, fc1_biases, fc2_weights, fc2_biases]

    all_params_names = ['conv1_weights', 'conv1_biases', 'conv2_weights', 
                                'conv2_biases', 'conv3_weights', 'conv3_biases',
    ...
    # Above modified for visulazation
    ...
    with tf.Session() as s:
        ...
        '''
        '''
        predict_data_filename = '/home/felipesulser/Desktop/ex9/road_segmentation/test_set_images';
